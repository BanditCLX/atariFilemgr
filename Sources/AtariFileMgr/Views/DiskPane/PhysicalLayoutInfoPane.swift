...
struct PhysicalLayoutInfoPane: View {
        ...
        return VStack(alignment: .leading, spacing: 2) {
            ...
            LazyVGrid(columns: columns, spacing: 1.5) {
                ForEach(0..<totalClusters, id: \.self) { index in
                    ...
                    let isSelected = vm.selectedEntryClusters.contains(cluster)
            ...
            .id(vm.selectedEntryClusters) // Forces immediate redraw when selection changes
        }
    }
    ...
    private func hoverText(fs: GEMDOSFilesystem) -> String {
        guard let clusterNum = hoveredCluster else { return "Hover cell" }
        let cluster = UInt16(clusterNum)
        
        let fatVal = fs.getFatEntry(for: cluster)
        if fatVal == 0x000 {
            return "Clust \(clusterNum): Free"
        } else if fatVal == 0xFF7 {
            return "Clust \(clusterNum): Bad"
        }
        ...
        if let result = vm.findEntry(forCluster: cluster) {
            ...
            return "Clust \(clusterNum): \(result.entry.displayName)\(sizeStr) · \(status)"
        }
        
        return "Clust \(clusterNum): Allocated"
    }
}
